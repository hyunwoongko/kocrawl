...
구조가 매우 간단하고 객체지향을 최대한 살려서 구현했기 때문에 소스코드를 보시면 
크롤러 개발 공부하시는데 도움이 될 수 있습니다. 이에 관련한 시퀀스 다이어그램을
아래에 첨부합니다.<br><br>
...
###  1. 해당 사이트가 정적 사이트일때
- beautifulsoup과 CSS 셀렉터로 구현합니다.
- Searcher에서 CSS 셀렉터를 정의하고 bs4로 검색을 수행합니다.
- 셀렉터 딸 때는 구글크롬을 사용하는 것이 효과적입니다.
...
###  2. 해당 사이트가 동적 사이트(ajax) 일때
- 동적 사이트의 경우 HTTP 통신 중 GET을 사용한다면 매우 쉽게 크롤링 할 수 있습니다.
- 만약 POST를 사용한다면 Selenium 같이 실제로 브라우저를 띄우고 크롤링 하는 방식을 사용해야 
하는데 Kocrawl은 이런 방식의 크롤링은 다루지 않을 예정입니다. 여기에서는 GET 방식 크롤링만
알려드리도록 하겠습니다. 
- 이번에는 구글크롬의 Network를 이용합니다. 구글크롬을 열어서 F12를 눌러서 개발자 도구를 열고, Network를 눌러봅시다.
...
필수적으로 필요한 파라미터만 남기고 다 없애는 것이 좋습니다.

![](docs/impl_07.jpg)
- qeury를 제외한 모든 파라미터를 날려도 정상적으로 작동하기 때문에 qeury만 남깁니다.
- 즉, `https://map.naver.com/v5/api/search?query=` 뒤에 사용자 입력 쿼리를 붙여서 검색하면 됩니다. 
- 이 url을 이용해 파이썬에서 requests 모듈을 사용해서 접속하고, json 모듈을 통해서 원하는 정보에 접근하면 됩니다.
...
저와 함께 힘겨운 크롤링의 여정을 떠나실 준비가 되신 분들이라면 얼마든지 환영합니다. ^^...
<br><br>
